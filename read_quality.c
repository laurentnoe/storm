...
inline int fastq_parse_quality_from_char(char c) {
  ...
  if (value < 0) {
    ERROR__(("Quality level char \"%c\" {%x} (from base \"%c\" {%x}) induces a negative value \"%d\" .\nUnable to set quality value.\n", c, (int)c, (char)read_quality_min_symbol_code, read_quality_min_symbol_code, value));
    exit (RETURN_INPUT_ERR);
  } else {
    if ( value >= MAX_READ_QUALITY) {
      ERROR__(("Quality level char \"%c\" {%x} (from base \"%c\" {%x}) induces a too large value \"%d\" .\nUnable to set quality value.\n", c, (int)c, (char)read_quality_min_symbol_code, read_quality_min_symbol_code, value));
      exit (RETURN_INPUT_ERR);
    }
  }
  return value;
